...
|-------|-------------------|----------|
| **Skill** | Outcome type, story, art direction, `themeId`, `layoutId`, slot copy, when to clarify | Author x/y/w/h, dump catalogs into the system prefix, invent a second file to dodge QA |
| **Prompt** | Compact invariants: open canvas is the target; one task = one artifact unless the user asks otherwise; deck/poster normal path is semantic theme/layout/slots; QA failure must be repaired on that file | Layout catalogs, lengthy recipes, host isolation law |
| **Runtime** | Bind the live `artifactId`, compile geometry, gate writes, return `CANVAS_QA_FAILED` with issues | Ask the prompt to enforce isolation or pagination |
...
- After a canvas exists: call `deck` / `web` / `sheet` / `doc` instead of describing the edit in chat
