...
6j. **Plan approval panel** — same yield host (`#clarifyLive.is-plan`), different chrome: `.plan-panel` (elevated card, not a question pop-card). Steps with `detail` are `<details>` rows (title + chevron; collapsed by default). Title-only steps stay flat. Footer is Decline (secondary) + Required to change / 需要修改 (secondary) + Approve (primary). Required to change opens an inline notes field; submit stays disabled until notes are non-empty. After approve/decline, **seal in place** (drop `#clarifyLive` id, drop actions, kicker becomes 已批准 / 未批准). After revise, seal as 需修改 with the notes receipt visible; the next plan card stacks below — do not vanish the previous plan the way clarify questions vanish. Tokens only. Respect `prefers-reduced-motion`.
